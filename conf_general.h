...
// This is how to provide a custom UI in VESC Tool. The UI can be created and
// tested in the scripting page, then the source files can be exported. The
// defines below use the exported files to provide the custom UI when VESC Tool
// connects.
//
// The intention if the HW gui is to be part of the HW-file and the app gui is
// for custom apps. Both can be used at the same time.
//
// Defining QMLUI_HW_FULLSCREEN and/or QMLUI_APP_FULLSCREEN will disable the
// other pages in the mobile version of VESC Tool.
//
...
/*
 * Set APP_CUSTOM_TO_USE to the name of the main C file of the custom
 * application.
 */
...
// Correction factor for computations that depend on the old resistor division
// factor
...
// #define V_REG						(1.21 /
// ((float)ADC_Value[ADC_IND_VREFINT] / 4095.0)) #define V_REG
// 3.3

// Use the pins for the hardware SPI port instead of the hall/encoder pins for
// the AS5047
...
/*
 *	Run the BLDC speed controller in current mode instead of duty cycle
 *mode. This will make it behave like the FOC speed controller. The duty cycle
 *mode has the advantage that it does not require the extra current controller
 *since bldc inherently runs with duty cycle control. The current controller
 *also outputs a duty cycle in the end, and then the speed controller might as
 *well do the same without the current controller dynamics in between. FOC on
 *the other hand is inherently based on current control.
 */
#define BLDC_SPEED_CONTROL_CURRENT 1

/*
 *	Run the FOC loop once every N ADC ISR requests. This way the pwm
 *frequency is detached from the FOC calculation, which because it takes ~25usec
 *it can't work at >40khz. To set a 100kHz pwm FOC_CONTROL_LOOP_FREQ_DIVIDER can
 *be set at 3 so it skips 2 ISR calls and execute the control loop in the 3rd
 *call.
 */
...
bool conf_general_detect_motor_param(float   current,
                                     ...
                                     int8_t *hall_table,
                                     int    *hall_res);
bool conf_general_measure_flux_linkage(
    float current, float duty, float min_erpm, float res, float *linkage);
uint8_t conf_general_calculate_deadtime(float deadtime_ns,
                                        float core_clock_freq);
int     conf_general_measure_flux_linkage_openloop(float  current,
                                                   float  duty,
                                                   float  erpm_per_sec,
                                                   float  res,
                                                   float  ind,
...
int     conf_general_autodetect_apply_sensors_foc(float current,
                                                  ...
                                                  int  *result);
void    conf_general_calc_apply_foc_cc_kp_ki_gain(mc_configuration *mcconf,
                                                  float             tc);
int     conf_general_detect_apply_all_foc(float max_power_loss,
                                          bool  store_mcconf_on_success,
                                          bool  send_mcconf_on_success);
int     conf_general_detect_apply_all_foc_can(
        bool  detect_can,
