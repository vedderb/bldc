...
static void
lib_sleep_ms (uint32_t ms)
{
    chThdSleepMilliseconds(ms);
}

static void
lib_sleep_us (uint32_t us)
{
    chThdSleepMicroseconds(us);
}

static float
lib_system_time (void)
{
...
lib_thread
lispif_spawn (void (*func)(void *), size_t stack_size, char *name, void *arg)
{
    ...
    if (mem)
    {
        ...
        if (info)
        {
            ...
            thread_t *thd
                = chThdCreateStatic(mem, stack_size, NORMALPRIO, lib_thd, info);

            if (lib_running_threads_cnt < (sizeof(lib_running_threads)
...
static void
lib_request_terminate (lib_thread thd)
{
...
static bool
lib_should_terminate (void)
{
    return chThdShouldTerminateX();
}

static void **
lib_get_arg (uint32_t prog_addr)
{
...
static void
lib_set_pad_mode (void *gpio, uint32_t pin, uint32_t mode)
{
    palSetPadMode((stm32_gpio_t *)gpio, pin, mode);
}

static void
lib_set_pad (void *gpio, uint32_t pin)
{
    palSetPad((stm32_gpio_t *)gpio, pin);
}

static void
lib_clear_pad (void *gpio, uint32_t pin)
{
    palClearPad((stm32_gpio_t *)gpio, pin);
}

static bool
get_gpio (VESC_PIN io, stm32_gpio_t **port, uint32_t *pin, bool *is_analog)
{
    bool res   = false;
    *is_analog = false;
...
static bool
lib_io_set_mode (VESC_PIN pin_vesc, VESC_PIN_MODE mode)
{
    stm32_gpio_t *gpio;
    uint32_t      pin;
    bool          is_analog;
...
static bool
lib_io_write (VESC_PIN pin_vesc, int state)
{
    stm32_gpio_t *gpio;
    uint32_t      pin;
    bool          is_analog;
...
static float
lib_io_read_analog (VESC_PIN pin_vesc)
{
    float res = -1.0;
...
static bool
lib_io_get_st_pin (VESC_PIN vesc_pin, void **gpio, uint32_t *pin)
{
    bool analog;
...
static bool
lib_symbol_to_io (lbm_uint sym, void **gpio, uint32_t *pin)
{
    return lispif_symbol_to_io(sym, (stm32_gpio_t **)gpio, pin);
}

static SerialConfig uart_cfg = { 2500000, 0, 0, 0 };

static bool
lib_uart_start (uint32_t baudrate, bool half_duplex)
{
    app_configuration *appconf = mempools_alloc_appconf();
    conf_general_read_app_configuration(appconf);
    if (appconf->app_to_use == APP_UART || appconf->app_to_use == APP_PPM_UART
    ...
    palSetPadMode(
        HW_UART_TX_PORT, HW_UART_TX_PIN, PAL_MODE_ALTERNATE(HW_UART_GPIO_AF));
    if (!half_duplex)
    {
        palSetPadMode(HW_UART_RX_PORT,
                      HW_UART_RX_PIN,
...
static bool
lib_uart_write (uint8_t *data, uint32_t size)
{
...
static float
lib_ts_to_age_s (systime_t ts)
{
    return UTILS_AGE_S(ts);
}

static float
lib_get_cfg_float (CFG_PARAM p)
{
    float res = 0.0;
    ...
    switch (p)
    {
        ...
        case CFG_PARAM_si_gear_ratio:
            res = mcconf->si_gear_ratio;
            break;
        case CFG_PARAM_si_wheel_diameter:
            res = mcconf->si_wheel_diameter;
            break;
        case CFG_PARAM_si_battery_ah:
            res = mcconf->si_battery_ah;
            break;
        case CFG_PARAM_si_motor_nl_current:
            res = mcconf->si_motor_nl_current;
            break;
...
lib_get_cfg_int (CFG_PARAM p)
{
    ...
    switch (p)
    {
        case CFG_PARAM_app_can_mode:
            res = appconf->can_mode;
            break;
        case CFG_PARAM_app_can_baud_rate:
            res = appconf->can_baud_rate;
            break;
        case CFG_PARAM_app_shutdown_mode:
            res = appconf->shutdown_mode;
            break;

        case CFG_PARAM_si_motor_poles:
            res = mcconf->si_motor_poles;
            break;
        case CFG_PARAM_si_battery_type:
            res = mcconf->si_battery_type;
            break;
        case CFG_PARAM_si_battery_cells:
            res = mcconf->si_battery_cells;
            break;
        default:
            break;
    }
...
static bool
lib_set_cfg_float (CFG_PARAM p, float value)
{
    bool res = false;
    ...
    *appconf                   = *app_get_configuration();
    ...
    // Safe changes that can be done instantly on the pointer. It is not that
    // good to do it this way, but it is much faster.
    ...
    switch (p)
    {
        ...
        case CFG_PARAM_si_gear_ratio:
            mcconf->si_gear_ratio = value;
            changed_mc            = 1;
            res                   = true;
            break;
        case CFG_PARAM_si_wheel_diameter:
            mcconf->si_wheel_diameter = value;
            changed_mc                = 1;
            res                       = true;
            break;
        case CFG_PARAM_si_battery_ah:
            mcconf->si_battery_ah = value;
            changed_mc            = 1;
            res                   = true;
            break;
        case CFG_PARAM_si_motor_nl_current:
            mcconf->si_motor_nl_current = value;
            changed_mc                  = 1;
            res                         = true;
            break;
        default:
...
static bool
lib_set_cfg_int (CFG_PARAM p, int value)
{
    bool res = false;
    ...
    *appconf                   = *app_get_configuration();
    ...
    switch (p)
    {
        ...
        case CFG_PARAM_app_can_baud_rate:
            ...
            break;
        case CFG_PARAM_app_shutdown_mode:
            appconf->shutdown_mode = value;
            changed_app            = 1;
            res                    = true;
            break;

        case CFG_PARAM_si_motor_poles:
            mcconf->si_motor_poles = value;
            changed_mc             = 1;
            res                    = true;
            break;
        case CFG_PARAM_si_battery_type:
            mcconf->si_battery_type = value;
            changed_mc              = 1;
            res                     = true;
            break;
        case CFG_PARAM_si_battery_cells:
            mcconf->si_battery_cells = value;
            changed_mc               = 1;
            res                      = true;
            break;
        default:
...
lib_store_cfg (void)
{
    ...
    *mcconf                  = *mc_interface_get_configuration();
    bool res_mc              = conf_general_store_mc_configuration(
...
static bool
lib_create_byte_array (lbm_value *value, lbm_uint num_elt)
{
    return lbm_heap_allocate_array(value, num_elt);
}

static bool
lib_eval_is_paused (void)
{
    return lbm_get_eval_state() == EVAL_CPS_STATE_PAUSED;
}

static lib_mutex
lib_mutex_create (void)
{
...
static void
lib_sem_wait (lib_semaphore s)
{
    chSemWait((semaphore_t *)s);
}

static void
lib_sem_signal (lib_semaphore s)
{
    chSemSignal((semaphore_t *)s);
}

static bool
lib_sem_wait_to (lib_semaphore s, systime_t timeout_ticks)
{
    return chSemWaitTimeout((semaphore_t *)s, timeout_ticks) == MSG_OK;
}

static void
lib_sem_reset (lib_semaphore s)
{
    chSemReset((semaphore_t *)s, 0);
}

static remote_state
lib_get_remote_state (void)
{
    remote_state res;
...
static float
lib_get_ppm_age (void)
{
    return (float)servodec_get_time_since_update() / 1000.0;
}

static bool
lib_add_extension (char *sym_str, extension_fptr ext)
{
...
static int
lib_lbm_set_error_reason (char *str)
{
...
lbm_value
ext_load_native_lib (lbm_value *args, lbm_uint argn)
{
    ...
    if (!lib_init_done)
    {
        ...
        cif.cif.mc_get_sampling_frequency_now
            = mc_interface_get_sampling_frequency_now;
        ...
        cif.cif.mc_get_tot_current_filtered
            = mc_interface_get_tot_current_filtered;
        cif.cif.mc_get_tot_current_directional
            = mc_interface_get_tot_current_directional;
        ...
        cif.cif.mc_get_tot_current_in_filtered
        ...
        cif.cif.mc_get_tachometer_abs_value
            = mc_interface_get_tachometer_abs_value;
        ...
        cif.cif.foc_set_openloop_duty_phase
            = mc_interface_set_openloop_duty_phase;
    ...
    for (int i = 0; i < LIB_NUM_MAX; i++)
    {
        if (loaded_libs[i].stop_fun == NULL)
        {
            ...
            if (loaded_libs[i].stop_fun != NULL
                && !utils_is_func_valid(loaded_libs[i].stop_fun))
            {
                loaded_libs[i].stop_fun = NULL;
                lbm_set_error_reason(
                    "Invalid stop function. Make sure that it is static.");
                return res;
            }
...
lbm_value
ext_unload_native_lib (lbm_value *args, lbm_uint argn)
{
    lbm_value res = lbm_enc_sym(SYM_EERROR);
...
lispif_get_ppm (void)
{
    ...
    servodec_set_pulse_options(
        cfg->pulse_start, cfg->pulse_end, cfg->median_filter);

    float servo_val = servodec_get_servo(0);
    float servo_ms
        = utils_map(servo_val, -1.0, 1.0, cfg->pulse_start, cfg->pulse_end);

    // Mapping with respect to center pulsewidth
    if (servo_ms < cfg->pulse_center)
    {
        servo_val = utils_map(
            servo_ms, cfg->pulse_start, cfg->pulse_center, -1.0, 0.0);
    }
    else
    {
        servo_val
            = utils_map(servo_ms, cfg->pulse_center, cfg->pulse_end, 0.0, 1.0);
    }
