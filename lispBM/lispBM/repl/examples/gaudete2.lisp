...
(defun strength-to-velocity (strength)
  (cond ((eq strength 'strong) 1.0)
        ((eq strength 'medium) 0.9)
        ((eq strength 'weak) 0.8)
        (t 90)))

(defun cmp-events (a b)
  (< (car a) (car b)))

(defmacro close (args body vars)
  (let ((env (map (lambda (x) (cons x (eval x))) vars))) {
...
(defun bar-to-events (bar bpm patch-no t-zero) {
       ...
       (loopforeach b (bar-notes bar) {
             ...
             (var vol-factor (strength-to-velocity (ix strengths beat-idx)))
             ...
             (loopforeach k b { ;; For each note in chord
                   ...
                   (if (not (eq (cdr k) 'rest))
                       {
                       ;; This illustrates an issue with the cheating in closure construction.
                       ;; the closures created below will keep more things aline than neccessary and
                       ;; increase the memory requirements of this application artificially.
                       ;;(setq s-events (cons (list t-now (lambda (vol) (note-on patch-no (cdr k) (to-i (* vol vol-factor))))) s-events))
                       ;;(setq e-events (cons (list (+ t-now dur) (lambda (vol) (note-off patch-no (cdr k)))) e-events))

                       ;; Explicitly closing with the known subenvironment
                       ;(var clo-env `((patch-no . ,patch-no) (k . ,k) (vol-factor . ,vol-factor)))
                       (setq s-events
                             (cons
                              (list t-now
                                    (close (vol) (note-on patch-no (cdr k) (to-i (* vol vol-factor))) (patch-no k vol-factor))) s-events))
                       (setq e-events
                             (cons
                              (list (+ t-now dur)
                                    (close (vol) (note-off patch-no (cdr k)) (patch-no k vol-factor))) e-events))
                       }
                       nil)
                   })
...
(defun bars-to-events (bars bpm patch-no t-zero) {
       ...
       (cons t-now (sort cmp-events all-events))
       })


;; Evaluate events  ((t1 event) (t2 event) ... (tn event))
(defun play-events (evts time) {
       (loopwhile (and evts (> time (car (car evts)))) {
        ((car (cdr (car evts))) 127)
        (setq evts (cdr evts))
        })
       evts
       })
...
(defun play-tracks (tracks) {
       (var t0 (systime))
       (loopwhile (some tracks) {
        (var curr-time (- (systime) t0)) 
        (setq tracks (map (lambda (evts) (play-events evts curr-time)) tracks))
        })
       })
...
(defun play-chorus ()
  (play-tracks (list (cdr (bars-to-events r1 110 1 0))
                     (cdr (bars-to-events r2 110 0 0))
                     (cdr (bars-to-events r3 110 2 0)))))


(defun play-solo ()
  (play-tracks (list (cdr (bars-to-events s1 110 1 0))
                     (cdr (bars-to-events s2 110 0 0))
                     (cdr (bars-to-events s3 110 2 0)))))
