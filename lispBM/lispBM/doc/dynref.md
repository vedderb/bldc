...
```clj
(macro (a) (append '(list) (list ''apa) (list a)))
```
...
Structs are implemented as lists of lisp values. Each field is reached by a fixed number of list-traversal steps that is decided when the struct type is defined (by defstruct), not by a runtime search over the fields. 

As structs are built from ordinary heap cons cells rather than an array allocation (lbm_memory), creating and discarding struct instances does not risk causing memory fragmentation the way large array allocations can. 
