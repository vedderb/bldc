...
static volatile bool xmodem_active = false;

void done_callback(eval_context_t *ctx) {
  if (xmodem_active) return; /* suppress output while XMODEM stream is live */
  char *output = outbuf;
  lbm_value t = ctx->r;
...
lbm_value ext_print(lbm_value *args, lbm_uint argn) {
  if (xmodem_active) return lbm_enc_sym(SYM_TRUE);
...
static lbm_buffered_channel_state_t xmodem_chan_state;
static lbm_char_channel_t           xmodem_chan;

static bool xmodem_getc(uint8_t *c, int timeout_ms, void *arg) {
  (void)arg;
  msg_t r = chnGetTimeout((BaseChannel *)&SDU1, TIME_MS2I(timeout_ms));
  if (r == MSG_TIMEOUT || r == MSG_RESET) return false;
  *c = (uint8_t)r;
  return true;
}

static void xmodem_putc(uint8_t c, void *arg) {
  (void)arg;
  streamPut(chp, c);
}

static bool xmodem_data(const uint8_t *buf, unsigned int len, void *arg) {
  xmodem_ctx_t *ctx = (xmodem_ctx_t *)arg;
  for (unsigned int i = 0; i < len; i++) {
    int r;
    do {
      r = lbm_channel_write(ctx->chan, (char)buf[i]);
      if (r == CHANNEL_FULL) chThdSleepMicroseconds(500);
    } while (r == CHANNEL_FULL);
    if (r == CHANNEL_READER_CLOSED) return false;
  }
  return true;
}

static void do_xmodem_upload(void) {
  xmodem_active = true;
  lbm_pause_eval();
  ...
  lbm_create_buffered_char_channel(&xmodem_chan_state, &xmodem_chan);
  lbm_cid cid = lbm_load_and_eval_program_incremental(&xmodem_chan, "xmodem");
  lbm_continue_eval();

  chprintf(chp, "XMODEM ready - exit terminal and run sx.\r\n");

  xmodem_ctx_t ctx = { .chan = &xmodem_chan };
  lbm_xmodem_result_t r = lbm_xmodem_receive(xmodem_getc, xmodem_putc, xmodem_data, &ctx);

  lbm_channel_writer_close(&xmodem_chan);
  ...
  if (r == LBM_XMODEM_OK) {
    ...
    switch (r) {
    case LBM_XMODEM_ERR_TIMEOUT: reason = "timeout";  break;
    case LBM_XMODEM_ERR_SYNC:    reason = "sync error"; break;
    case LBM_XMODEM_ERR_CRC:     reason = "CRC error";  break;
    case LBM_XMODEM_ERR_CANCEL:  reason = "cancelled";  break;
    default: break;
    }
    chprintf(chp, "\r\nXMODEM error: %s\r\n", reason);
  }
