...
(define win (sdl-create-window "GetPixel Test - rgb565" 400 200))
...
;; Test 2: Set and get exact rgb565 values (these should match exactly)
...
(define black565 0x0000)   ; 00000 000000 00000 = black

(img-setpix img400x200 0 0 red565)
(define test2 (= (img-getpix img400x200 0 0) red565))

(img-setpix img400x200 10 0 green565)
(define test3 (= (img-getpix img400x200 10 0) green565))

(img-setpix img400x200 20 0 blue565)
(define test4 (= (img-getpix img400x200 20 0) blue565))

(img-setpix img400x200 30 0 white565)
(define test5 (= (img-getpix img400x200 30 0) white565))

(img-setpix img400x200 40 0 black565)
(define test6 (= (img-getpix img400x200 40 0) black565))

;; Test 7: Test values that may be transformed due to bit precision
;; Set non-exact rgb565 values and verify they get quantized properly
(img-setpix img400x200 50 10 0x8000)  ; Should become closest valid red value
(define test7_val (img-getpix img400x200 50 10))
(define test7 (and (>= test7_val 0) (<= test7_val 65535)))

(img-setpix img400x200 60 10 0x0400)  ; Should become closest valid green value
(define test8_val (img-getpix img400x200 60 10))
(define test8 (and (>= test8_val 0) (<= test8_val 65535)))

(img-setpix img400x200 70 10 0x0010)  ; Should become closest valid blue value
(define test9_val (img-getpix img400x200 70 10))
(define test9 (and (>= test9_val 0) (<= test9_val 65535)))
...
(img-setpix img400x200 100 50 0x8410)  ; Some gray value
(define test12a_val (img-getpix img400x200 100 50))
(define test12a (and (>= test12a_val 0) (<= test12a_val 65535)))
...
(img-setpix img400x200 150 75 red565)
(img-setpix img400x200 151 75 green565)
(img-setpix img400x200 152 75 blue565)
(define test17a (= (img-getpix img400x200 150 75) red565))
(define test17b (= (img-getpix img400x200 151 75) green565))
(define test17c (= (img-getpix img400x200 152 75) blue565))
(define test17 (and test17a test17b test17c))

;; Test 18: Test various intermediate values get quantized consistently
(img-setpix img400x200 200 100 0x5555)  ; Mixed value
(define test18_val (img-getpix img400x200 200 100))
(img-setpix img400x200 201 100 0x5555)  ; Same value again
(define test18_val2 (img-getpix img400x200 201 100))
(define test18 (= test18_val test18_val2))  ; Should be consistent

;; Test 19: Test some compound colors
...
(img-setpix img400x200 300 100 yellow565)
(define test19a (= (img-getpix img400x200 300 100) yellow565))

(img-setpix img400x200 310 100 magenta565)
(define test19b (= (img-getpix img400x200 310 100) magenta565))

(img-setpix img400x200 320 100 cyan565)
(define test19c (= (img-getpix img400x200 320 100) cyan565))

(define test19 (and test19a test19b test19c))
...
(if (and test1 test2
         ;;test3 test4 test5
         test6 test7 test8 test9 test12
         ;;test17
         test18
         ;;test19
         (eq test13 '(exit-error type_error))
