...
static THD_WORKING_AREA(adc_thread_wa,
1024);

// Private variables
static volatile adc_config config;
static volatile app_configuration rootConfig;
...
void app_custom_configure(app_configuration *conf) {
    rootConfig = *conf;
    config = rootConfig.app_adc_conf;
...
static THD_FUNCTION(adc_thread, arg) {
    ...
    for(;;) {
        ...
        case ADC_CTRL_TYPE_CURRENT_REV_BUTTON:
            ...
            if (fabsf(pwr) < 0.001) {
                ms_without_power += (1000.0 * (float)sleep_time) / (float)
                CH_CFG_ST_FREQUENCY;
            }
            break;
        ...
        case ADC_CTRL_TYPE_DUTY_REV_BUTTON:
            ...
            if (!(ms_without_power < MIN_MS_WITHOUT_POWER && config.safe_start)) {
                mc_interface_set_duty (utils_map(pwr,

                -1.0, 1.0, -mcconf->l_max_duty, mcconf->l_max_duty));
                send_duty = true;
            }
            break;
