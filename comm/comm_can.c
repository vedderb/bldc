...
comm_can_init (void)
{
    ...
    palSetPadMode(HW_CANRX_PORT,
                  HW_CANRX_PIN,
                  PAL_MODE_ALTERNATE(HW_CAN_GPIO_AF) | PAL_STM32_OTYPE_PUSHPULL
    ...
    palSetPadMode(HW_CAN2_RX_PORT,
                  HW_CAN2_RX_PIN,
                  PAL_MODE_ALTERNATE(HW_CAN2_GPIO_AF) | PAL_STM32_OTYPE_PUSHPULL
                      | PAL_STM32_OSPEED_MID1);
    palSetPadMode(HW_CAN2_TX_PORT,
                  HW_CAN2_TX_PIN,
                  PAL_MODE_ALTERNATE(HW_CAN2_GPIO_AF) | PAL_STM32_OTYPE_PUSHPULL
                      | PAL_STM32_OSPEED_MID1);
    ...
    chThdCreateStatic(cancom_read_thread_wa,
                      sizeof(cancom_read_thread_wa),
                      NORMALPRIO + 1,
                      cancom_read_thread,
                      NULL);
    chThdCreateStatic(cancom_status_thread_wa,
                      sizeof(cancom_status_thread_wa),
                      NORMALPRIO,
                      cancom_status_thread,
                      NULL);
    chThdCreateStatic(cancom_status_thread_2_wa,
                      sizeof(cancom_status_thread_2_wa),
                      NORMALPRIO,
                      cancom_status_thread_2,
                      NULL);
    chThdCreateStatic(cancom_process_thread_wa,
                      sizeof(cancom_process_thread_wa),
                      NORMALPRIO,
                      cancom_process_thread,
                      NULL);
#ifdef HW_HAS_DUAL_MOTORS
    chThdCreateStatic(cancom_status_internal_thread_wa,
                      sizeof(cancom_status_internal_thread_wa),
                      NORMALPRIO,
                      cancom_status_internal_thread,
                      NULL);
#endif
...
void
comm_can_set_baud (CAN_BAUD baud)
{
    switch (baud)
    {
        case CAN_BAUD_125K:
...
comm_can_transmit_eid_replace (
    uint32_t id, const uint8_t *data, uint8_t len, bool replace, int interface)
{
    if (len > 8)
    {
        len = 8;
    ...
    if (interface == 0)
    {
        for (int i = 0; i < 10; i++)
        {
            msg_t ok = canTransmit(
                &HW_CAN_DEV, CAN_ANY_MAILBOX, &txmsg, TIME_IMMEDIATE);
            msg_t ok2 = canTransmit(
                &HW_CAN2_DEV, CAN_ANY_MAILBOX, &txmsg, TIME_IMMEDIATE);
...
void
comm_can_transmit_eid (uint32_t id, const uint8_t *data, uint8_t len)
{
    comm_can_transmit_eid_replace(id, data, len, false, 0);
}

void
comm_can_transmit_eid_if (uint32_t       id,
                          const uint8_t *data,
                          uint8_t        len,
                          int            interface)
{
...
void
comm_can_transmit_sid (uint32_t id, const uint8_t *data, uint8_t len)
{
    if (len > 8)
    {
        len = 8;
    ...
    {
        msg_t ok
            = canTransmit(&HW_CAN_DEV, CAN_ANY_MAILBOX, &txmsg, TIME_IMMEDIATE);
        msg_t ok2 = canTransmit(
            &HW_CAN2_DEV, CAN_ANY_MAILBOX, &txmsg, TIME_IMMEDIATE);
        if (ok == MSG_OK || ok2 == MSG_OK)
        {
 ...
 *
 * The callback should return true if the frame was used by the application,
 * false otherwise. if the frame was used, no further processing will be done
 * here.
 *
 * @param p_func
 ...
 */
void
comm_can_set_sid_rx_callback (bool (*p_func)(uint32_t id,
                                             uint8_t *data,
                                             uint8_t  len))
{
    sid_callback = p_func;
 ...
 */
void
comm_can_set_eid_rx_callback (bool (*p_func)(uint32_t id,
                                             uint8_t *data,
                                             uint8_t  len))
{
    eid_callback = p_func;
...
/**
 * Send a buffer up to RX_BUFFER_SIZE bytes as fragments. If the buffer is 6
 * bytes or less it will be sent in a single CAN frame, otherwise it will be
 ...
 * 3: Same as 0, but the reply is processed locally and not sent out on the last
 * interface.
 */
void
comm_can_send_buffer (uint8_t      controller_id,
                      uint8_t     *data,
                      unsigned int len,
                      uint8_t      send)
{
    ...
    if (len <= 6)
    {
        ...
        ind += len;
        comm_can_transmit_eid_replace(
            controller_id | ((uint32_t)CAN_PACKET_PROCESS_SHORT_BUFFER << 8),
            send_buffer,
            ind,
            true,
            0);
    ...
    else
    {
        ...
        for (unsigned int i = end_a; i < len; i += 6)
        {
            ...
            comm_can_transmit_eid_replace(
                controller_id | ((uint32_t)CAN_PACKET_FILL_RX_BUFFER_LONG << 8),
                send_buffer,
                send_len + 2,
                true,
                0);
        ...
        comm_can_transmit_eid_replace(
            controller_id | ((uint32_t)CAN_PACKET_PROCESS_RX_BUFFER << 8),
            send_buffer,
            ind++,
            true,
            0);
...
void
comm_can_set_duty (uint8_t controller_id, float duty)
{
...
void
comm_can_set_current (uint8_t controller_id, float current)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
...
void
comm_can_set_current_off_delay (uint8_t controller_id,
                                float   current,
                                float   off_delay)
{
    int32_t send_index = 0;
...
void
comm_can_set_current_brake (uint8_t controller_id, float current)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    buffer_append_int32(buffer, (int32_t)(current * 1000.0), &send_index);
...
void
comm_can_set_pos (uint8_t controller_id, float pos)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
 ...
 */
void
comm_can_set_current_rel (uint8_t controller_id, float current_rel)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(
        controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_REL << 8),
        buffer,
        send_index,
        true,
        0);
 ...
 */
void
comm_can_set_current_rel_off_delay (uint8_t controller_id,
                                    float   current_rel,
                                    float   off_delay)
{
    int32_t send_index = 0;
 ...
 */
void
comm_can_set_current_brake_rel (uint8_t controller_id, float current_rel)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(
        controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE_REL << 8),
        buffer,
        send_index,
        true,
        0);
 ...
 */
void
comm_can_set_handbrake (uint8_t controller_id, float current)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    buffer_append_float32(buffer, current, 1e3, &send_index);
    comm_can_transmit_eid_replace(
        controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE << 8),
        buffer,
        send_index,
        true,
        0);
 ...
 */
void
comm_can_set_handbrake_rel (uint8_t controller_id, float current_rel)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(
        controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE_REL << 8),
        buffer,
        send_index,
        true,
        0);
 ...
 */
bool
comm_can_ping (uint8_t controller_id, HW_TYPE *hw_type)
{
#if CAN_ENABLE
    if (app_get_configuration()->can_mode != CAN_MODE_VESC)
    {
    ...
    comm_can_transmit_eid_replace(
        controller_id | ((uint32_t)CAN_PACKET_PING << 8), buffer, 1, true, 0);
 ...
 */
void
comm_can_detect_apply_all_foc (uint8_t controller_id,
                               ...
                               float   max_power_loss)
{
    ...
    comm_can_transmit_eid_replace(
        controller_id | ((uint32_t)CAN_PACKET_DETECT_APPLY_ALL_FOC << 8),
        buffer,
        send_index,
        true,
        0);
 ...
 */
void
comm_can_conf_current_limits (uint8_t controller_id,
                              bool    store,
                              float   min,
                              float   max)
{
 ...
 */
void
comm_can_conf_current_limits_in (uint8_t controller_id,
                                 bool    store,
                                 float   min,
                                 float   max)
{
 ...
 */
void
comm_can_conf_foc_erpms (uint8_t controller_id,
                         bool    store,
...
int
comm_can_detect_all_foc_res (unsigned int index)
{
...
int
comm_can_detect_all_foc_res_size (void)
{
    return detect_all_foc_res_index;
}

void
comm_can_detect_all_foc_res_clear (void)
{
    detect_all_foc_res_index = 0;
}

void
comm_can_conf_battery_cut (uint8_t controller_id,
                           bool    store,
                           float   start,
                           float   end)
{
...
void
comm_can_shutdown (uint8_t controller_id)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    comm_can_transmit_eid_replace(controller_id
 ...
 */
can_status_msg *
comm_can_get_status_msg_index (int index)
{
 ...
 */
can_status_msg *
comm_can_get_status_msg_id (int id)
{
    for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
    {
 ...
 */
can_status_msg_2 *
comm_can_get_status_msg_2_index (int index)
{
    if (index < CAN_STATUS_MSGS_TO_STORE)
    {
 ...
 */
can_status_msg_2 *
comm_can_get_status_msg_2_id (int id)
{
    for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
    {
 ...
 */
can_status_msg_3 *
comm_can_get_status_msg_3_index (int index)
{
    if (index < CAN_STATUS_MSGS_TO_STORE)
    {
 ...
 */
can_status_msg_3 *
comm_can_get_status_msg_3_id (int id)
{
    for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
    {
 ...
 */
can_status_msg_4 *
comm_can_get_status_msg_4_index (int index)
{
    if (index < CAN_STATUS_MSGS_TO_STORE)
    {
 ...
 */
can_status_msg_4 *
comm_can_get_status_msg_4_id (int id)
{
    for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
    {
 ...
 */
can_status_msg_5 *
comm_can_get_status_msg_5_index (int index)
{
    if (index < CAN_STATUS_MSGS_TO_STORE)
    {
 ...
 */
can_status_msg_5 *
comm_can_get_status_msg_5_id (int id)
{
    for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
    {
 ...
 */
can_status_msg_6 *
comm_can_get_status_msg_6_index (int index)
{
    if (index < CAN_STATUS_MSGS_TO_STORE)
    {
 ...
 */
can_status_msg_6 *
comm_can_get_status_msg_6_id (int id)
{
    for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
    {
...
io_board_adc_values *
comm_can_get_io_board_adc_1_4_index (int index)
{
...
io_board_adc_values *
comm_can_get_io_board_adc_1_4_id (int id)
{
...
io_board_adc_values *
comm_can_get_io_board_adc_5_8_index (int index)
{
...
io_board_adc_values *
comm_can_get_io_board_adc_5_8_id (int id)
{
...
comm_can_get_io_board_digital_in_index (int index)
{
    if (index < CAN_STATUS_MSGS_TO_STORE)
    {
...
io_board_digial_inputs *
comm_can_get_io_board_digital_in_id (int id)
{
...
void
comm_can_io_board_set_output_digital (int id, int channel, bool on)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    ...
    comm_can_transmit_eid_replace(
        id | ((uint32_t)CAN_PACKET_IO_BOARD_SET_OUTPUT_DIGITAL << 8),
        buffer,
        send_index,
        true,
        0);
...
void
comm_can_io_board_set_output_pwm (int id, int channel, float duty)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    ...
    comm_can_transmit_eid_replace(
        id | ((uint32_t)CAN_PACKET_IO_BOARD_SET_OUTPUT_PWM << 8),
        buffer,
        send_index,
        true,
        0);
...
psw_status *
comm_can_get_psw_status_index (int index)
{
    if (index < CAN_STATUS_MSGS_TO_STORE)
    {
...
psw_status *
comm_can_get_psw_status_id (int id)
{
    for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
    {
        if (psw_stat[i].id == id)
...
void
comm_can_psw_switch (int id, bool is_on, bool plot)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
...
void
comm_can_update_pid_pos_offset (int id, float angle_now, bool store)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    ...
    comm_can_transmit_eid_replace(
        id | ((uint32_t)CAN_PACKET_UPDATE_PID_POS_OFFSET << 8),
        buffer,
        send_index,
        true,
        0);
 ...
 */
CANRxFrame *
comm_can_get_rx_frame (int interface)
{
    CANRxFrame *res = NULL;
...
void
comm_can_send_status1 (uint8_t id, bool replace)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    ...
    buffer_append_int16(
        buffer,
        (int16_t)(mc_interface_get_tot_current_filtered() * 1e1),
        &send_index);
    buffer_append_int16(buffer,
                        (int16_t)(mc_interface_get_duty_cycle_now() * 1e3),
                        &send_index);
...
void
comm_can_send_status2 (uint8_t id, bool replace)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    buffer_append_int32(buffer,
                        (int32_t)(mc_interface_get_amp_hours(false) * 1e4),
                        &send_index);
    buffer_append_int32(
        buffer,
        (int32_t)(mc_interface_get_amp_hours_charged(false) * 1e4),
        &send_index);
...
void
comm_can_send_status3 (uint8_t id, bool replace)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    buffer_append_int32(buffer,
                        (int32_t)(mc_interface_get_watt_hours(false) * 1e4),
                        &send_index);
    buffer_append_int32(
        buffer,
        (int32_t)(mc_interface_get_watt_hours_charged(false) * 1e4),
        &send_index);
...
void
comm_can_send_status4 (uint8_t id, bool replace)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    buffer_append_int16(
        buffer, (int16_t)(mc_interface_temp_fet_filtered() * 1e1), &send_index);
    buffer_append_int16(buffer,
                        (int16_t)(mc_interface_temp_motor_filtered() * 1e1),
                        &send_index);
    buffer_append_int16(
        buffer,
        (int16_t)(mc_interface_get_tot_current_in_filtered() * 1e1),
        &send_index);
    buffer_append_int16(
        buffer, (int16_t)(mc_interface_get_pid_pos_now() * 50.0), &send_index);
...
void
comm_can_send_status5 (uint8_t id, bool replace)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    buffer_append_int32(
        buffer, mc_interface_get_tachometer_value(false), &send_index);
    buffer_append_int16(
        buffer,
        (int16_t)(mc_interface_get_input_voltage_filtered() * 1e1),
        &send_index);
...
void
comm_can_send_status6 (uint8_t id, bool replace)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
...
static THD_FUNCTION(cancom_read_thread, arg)
{
    ...
    while (!chThdShouldTerminateX())
    {
        ...
        msg_t result
            = canReceive(&HW_CAN_DEV, CAN_ANY_MAILBOX, &rxmsg, TIME_IMMEDIATE);
        ...
        while (result == MSG_OK)
        {
            ...
            result = canReceive(
                &HW_CAN_DEV, CAN_ANY_MAILBOX, &rxmsg, TIME_IMMEDIATE);
        }

#ifdef HW_CAN2_DEV
        result
            = canReceive(&HW_CAN2_DEV, CAN_ANY_MAILBOX, &rxmsg, TIME_IMMEDIATE);
        ...
        while (result == MSG_OK)
        {
            ...
            result = canReceive(
                &HW_CAN2_DEV, CAN_ANY_MAILBOX, &rxmsg, TIME_IMMEDIATE);
        }
#endif
    }
    ...
    for (;;)
    {
                 ...
                 || app_get_configuration()->can_mode == CAN_MODE_UNUSED)
        {
            ...
            while ((rxmsg_tmp = comm_can_get_rx_frame(0)) != 0)
            {
                ...
                {
                    commands_fwd_can_frame(rxmsg.DLC,
                                           rxmsg.data8,
                                           rxmsg.IDE == CAN_IDE_EXT ? rxmsg.EID
                                                                    : rxmsg.SID,
                ...
                if (rxmsg.IDE == CAN_IDE_STD)
                {
                    ...
                    {
                        lispif_process_can(
                            rxmsg.SID, rxmsg.data8, rxmsg.DLC, false);
                    }
#else
                    (void)sid_cb_used;
#endif
                ...
                else
                {
                    ...
                    {
                        lispif_process_can(
                            rxmsg.EID, rxmsg.data8, rxmsg.DLC, true);
                    }
#else
                    (void)eid_cb_used;
#endif
        ...
        while ((rxmsg_tmp = comm_can_get_rx_frame(0)) != 0)
        {
            ...
            else
            {
                ...
                if (!sid_cb_used)
                {
                    sid_cb_used = bms_process_can_frame(
                        rxmsg.SID, rxmsg.data8, rxmsg.DLC, false);
                }
...
static void
send_can_status (uint8_t msgs, uint8_t id)
{
    if ((msgs >> 0) & 1)
    {
...
static void
send_packet_wrapper (unsigned char *data, unsigned int len)
{
    comm_can_send_buffer(rx_buffer_last_id, data, len, rx_buffer_response_type);
}

static void
decode_msg (uint32_t eid, uint8_t *data8, int len, bool is_replaced)
{
    int32_t ind = 0;
    uint8_t crc_low;
    uint8_t crc_high;
    ...
    if (id == 255 || id == id1 || id == id2)
    {
        switch (cmd)
        {
            ...
            case CAN_PACKET_SET_CURRENT:
                ...
                if (len >= 6)
                {
                    mc_interface_set_current_off_delay(
                        buffer_get_float16(data8, 1e3, &ind));
                }
            ...
            case CAN_PACKET_SET_CURRENT_BRAKE:
                ...
                mc_interface_set_brake_current(
                    buffer_get_float32(data8, 1e3, &ind));
                timeout_reset();
                break;
            ...
            case CAN_PACKET_SET_RPM:
                ...
                mc_interface_set_pid_speed(
                    buffer_get_float32(data8, 1e0, &ind));
                timeout_reset();
                break;
            ...
            case CAN_PACKET_PROCESS_RX_BUFFER: {
                        ...
                        | (unsigned short)crc_low))
                {
                    ...
                    switch (commands_send)
                    {
                        case 0:
                        case 3:
                            commands_process_packet(rx_buffer[buf_ind],
                                                    rxbuf_len,
                                                    send_packet_wrapper);
                            break;
                        case 1:
                            commands_send_packet_can_last(rx_buffer[buf_ind],
                                                          rxbuf_len);
                            break;
                        case 2:
                            commands_process_packet(
                                rx_buffer[buf_ind], rxbuf_len, 0);
                            break;
                        default:
                            break;
                    }
            ...
            case CAN_PACKET_PROCESS_SHORT_BUFFER: {
                ...
                switch (commands_send)
                {
                    ...
                    case 3:
                        commands_process_packet(
                            data8 + ind, len - ind, send_packet_wrapper);
                        break;
                    case 1:
            ...
            case CAN_PACKET_SET_CURRENT_REL:
                ...
                mc_interface_set_current_rel(
                    buffer_get_float32(data8, 1e5, &ind));
            ...
            case CAN_PACKET_SET_CURRENT_BRAKE_REL:
                ind = 0;
                mc_interface_set_brake_current_rel(
                    buffer_get_float32(data8, 1e5, &ind));
                timeout_reset();
                break;
            ...
            case CAN_PACKET_SET_CURRENT_HANDBRAKE_REL:
                ind = 0;
                mc_interface_set_handbrake_rel(
                    buffer_get_float32(data8, 1e5, &ind));
                timeout_reset();
                break;
            ...
            case CAN_PACKET_PING: {
                ...
                comm_can_transmit_eid_replace(
                    data8[0] | ((uint32_t)CAN_PACKET_PONG << 8),
                    buffer,
                    2,
                    true,
                    0);
            ...
            case CAN_PACKET_DETECT_APPLY_ALL_FOC: {
                ...
                int   res             = conf_general_detect_apply_all_foc(
                    max_power_loss, true, false);
            ...
            case CAN_PACKET_DETECT_APPLY_ALL_FOC_RES: {
                ...
                detect_all_foc_res[detect_all_foc_res_index++]
                    = (int8_t)data8[0];
                    ...
                    || mcconf->l_current_max != max)
                {
                    ...
                    if (cmd == CAN_PACKET_CONF_STORE_CURRENT_LIMITS)
                    {
                        conf_general_store_mc_configuration(
                            mcconf, mc_interface_get_motor_thread() == 2);
                    }
            ...
            case CAN_PACKET_CONF_STORE_CURRENT_LIMITS_IN: {
                ...
                *mcconf                  = *mc_interface_get_configuration();
                ...
                if (mcconf->l_in_current_min != min
            ...
            case CAN_PACKET_CONF_STORE_BATTERY_CUT: {
                ...
                *mcconf                  = *mc_interface_get_configuration();
                ...
                if (mcconf->l_battery_cut_start != start
            ...
            case CAN_PACKET_POLL_ROTOR_POS: {
                ...
                int32_t index = 0;
                buffer_append_int32(
                    buffer, (int32_t)(encoder_read_deg() * 100000.0), &index);
                comm_can_transmit_eid_replace(
                    app_get_configuration()->controller_id
    ...
    // The packets below are addressed to all devices, mainly containing status
    // information.
        ...
        case CAN_PACKET_STATUS:
            for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
            {
                ...
                if (stat_tmp->id == id || stat_tmp->id == -1)
                {
                    ...
                    stat_tmp->current
                        = (float)buffer_get_int16(data8, &ind) / 10.0;
                    stat_tmp->duty
                        = (float)buffer_get_int16(data8, &ind) / 1000.0;
                    break;
                }
            }
            break;
        ...
        case CAN_PACKET_STATUS_2:
            for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
            {
                ...
                if (stat_tmp_2->id == id || stat_tmp_2->id == -1)
                {
                    ...
                    stat_tmp_2->amp_hours
                        = (float)buffer_get_int32(data8, &ind) / 1e4;
                    stat_tmp_2->amp_hours_charged
                        = (float)buffer_get_int32(data8, &ind) / 1e4;
                    break;
                }
            }
        ...
        case CAN_PACKET_STATUS_3:
            for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
            {
                ...
                if (stat_tmp_3->id == id || stat_tmp_3->id == -1)
                {
                    ...
                    stat_tmp_3->watt_hours
                        = (float)buffer_get_int32(data8, &ind) / 1e4;
                    stat_tmp_3->watt_hours_charged
                        = (float)buffer_get_int32(data8, &ind) / 1e4;
                    break;
                }
            }
        ...
        case CAN_PACKET_STATUS_4:
            for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
            {
                ...
                if (stat_tmp_4->id == id || stat_tmp_4->id == -1)
                {
                    ...
                    stat_tmp_4->temp_fet
                        = (float)buffer_get_int16(data8, &ind) / 10.0;
                    stat_tmp_4->temp_motor
                        = (float)buffer_get_int16(data8, &ind) / 10.0;
                    stat_tmp_4->current_in
                        = (float)buffer_get_int16(data8, &ind) / 10.0;
                    stat_tmp_4->pid_pos_now
                        = (float)buffer_get_int16(data8, &ind) / 50.0;
                    break;
                }
            }
            break;
        ...
        case CAN_PACKET_STATUS_5:
            for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
            {
                ...
                if (stat_tmp_5->id == id || stat_tmp_5->id == -1)
                {
                    ...
                    stat_tmp_5->v_in
                        = (float)buffer_get_int16(data8, &ind) / 1e1;
                    break;
                }
            }
            break;
        ...
        case CAN_PACKET_IO_BOARD_ADC_1_TO_4:
            for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
            {
                ...
                if (msg->id == id || msg->id == -1)
                {
                    ...
                    {
                        msg->adc_voltages[j++]
                            = buffer_get_float16(data8, 1e2, &ind);
                    }
                    break;
                }
            }
 ...
 */
static void
set_timing (int brp, int ts1, int ts2)
{
    brp &= 0b1111111111;
    ts1 &= 0b1111;
    ts2 &= 0b111;

    cancfg.btr = CAN_BTR_SJW(3) | CAN_BTR_TS2(ts2) | CAN_BTR_TS1(ts1)
                 | CAN_BTR_BRP(brp);
