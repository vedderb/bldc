...
//				curr1 = -(curr0 + curr2);
	...
	UTILS_LP_FAST(state_now->v_bus, GET_INPUT_VOLTAGE(), 0.05);
	...
	if (motor_now->m_state == MC_STATE_RUNNING) {
		if (full_clarke) {
			...
			state_now->i_alpha = (2.0 / 3.0) * curr0 - (1.0 / 3.0) * curr1 - (1.0 / 3.0) * curr2;
			state_now->i_beta = ONE_BY_SQRT3 * curr1 - ONE_BY_SQRT3 * curr2;
			//filtered values
			state_now->i_alpha_filter = (2.0 / 3.0) * ia - (1.0 / 3.0) * ib - (1.0 / 3.0) * ic;
			state_now->i_beta_filter = ONE_BY_SQRT3 * ib - ONE_BY_SQRT3 * ic;
		} else {
			// Clarke transform assuming balanced currents
			state_now->i_alpha = curr0;
			state_now->i_beta = ONE_BY_SQRT3 * curr0 + TWO_BY_SQRT3 * curr1;
			//filtered values
			state_now->i_alpha_filter = ia;
			state_now->i_beta_filter = ONE_BY_SQRT3 * ia + TWO_BY_SQRT3 * ib;
		}
		...
		if (motor_now->m_i_alpha_beta_has_offset) {
			...
			state_now->i_alpha_filter = 0.5 * (state_now->i_alpha_filter + motor_now->m_i_alpha_sample_next);
			state_now->i_beta_filter = 0.5 * (state_now->i_beta_filter + motor_now->m_i_beta_sample_next);
		...
		// Set motor phase
		{
			if (!motor_now->m_phase_override) {
				foc_observer_update(state_now->v_alpha, state_now->v_beta,
						state_now->i_alpha_filter, state_now->i_beta_filter,
...
//		id_set_tmp -= motor_now->m_i_fw_set;
		...
		state_now->i_alpha_filter = 0.0;
		state_now->i_beta_filter = 0.0;
		state_now->id = 0.0;
		state_now->iq = 0.0;
		...
		// Run observer
		foc_observer_update(state_now->v_alpha, state_now->v_beta, state_now->i_alpha_filter, state_now->i_beta_filter,
