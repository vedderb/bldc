...
- blindly uses a switchable filter for current sensors, which must be disabled with low side shunts.
- uses 100v mosfets/caps. so so good to ~80v.
- only has 1 mosfet temperature sensor
- generally has a very poor current path so it definately isn't reaching 300a continuous.
- no aux output
*/
...
// current filter MUST be disabled for low side mosfets.
